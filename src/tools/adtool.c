...
void usage() {
	printf(
		...
		"groupadduser       <group> <user> [<container>]    add a user located in a given container to a group\n"
		"groupremoveuser    <group> <user> [<container>]    remove a user located in given container from a group\n"
		...
		"\n"
		"containercreate    <container name> <container>\n"
		"                                                   create a new container object\n"
		"containerdelete    <container name>                delete a container\n"
		"\n"
...
void groupadduser(char **argv) {
	...
	user=argv[1];
        ...
        if (strlen(container) > 0) {
                dn_length=strlen(user)+strlen(container)+5;
                dn=malloc(dn_length);
                snprintf(dn, dn_length, "cn=%s,%s", user, container);
                user_dn=ad_search("distinguishedName", dn);
        } else {
...
void containercreate(char **argv) {
        char *co, *container;
        int result, dn_length;
        char *dn;
        ...
        dn_length=strlen(co)+strlen(container)+5;
        dn=malloc(dn_length);
        snprintf(dn, dn_length, "cn=%s,%s", co, container);
        result=ad_co_create(co, dn);
        if(result!=AD_SUCCESS) {
                fprintf(stderr, "error: %s\n", ad_get_error());
                exit(1);
        }
...
void containerdelete(char **argv){
        char *co;
        int result;
        char **dn;
        ...
        result=ad_object_delete(*dn);
        if(result!=AD_SUCCESS) {
                fprintf(stderr, "error, container %s could not be deleted:\n%s\n", *dn, ad_get_error());
                exit(1);
        }
}
...
struct function function_table[] = {
	...
	{"groupadduser", groupadduser, 3},

	{"groupremoveuser", groupremoveuser, 3},
	...
	{"containercreate", containercreate, 2},

	{"containerdelete", containerdelete, 1},
